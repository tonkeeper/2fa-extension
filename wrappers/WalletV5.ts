...
export class WalletId {
    ...
    static deserialize(walletId: bigint | Buffer): WalletId {
        const bitReader = new BitReader(
            new BitString(typeof walletId === 'bigint' ? Buffer.from(walletId.toString(16), 'hex') : walletId, 0, 80),
        );
...
export class WalletV5 implements Contract {
    ...
    async sendInternalMessageFromExtension(
        ...
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(Opcodes.auth_extension, 32).storeSlice(opts.body.beginParse()).endCell(),
        });
    }
    ...
    async getExtensionsArray(provider: ContractProvider) {
        ...
        return dict.keys().map((key) => {
            ...
            let addr_hex = add_trailing_zeros_to_64_bytes(addressHex.toString(16));
            return Address.parseRaw(`${wc}:${addr_hex}`);
        });
    }
}

function add_trailing_zeros_to_64_bytes(str: string) {
    if (str.length == 64) {
        return str;
    }
